...
export default function ProjectList({ projects }) {
    if (!projects || projects.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-10 bg-white dark:bg-surface-dark rounded-xl border border-gray-100 dark:border-none shadow-sm">
                ...
                <Link href="/projects/new" className="px-6 py-2 bg-primary text-primary-foreground font-bold rounded-lg hover:bg-primary/90 transition-colors">
                    Criar Projeto
                </Link>
            </div>
        );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
                <Link href={`/projects/${project.id}`} key={project.id} className="group">
                    <div className="bg-white dark:bg-surface-dark rounded-xl border border-gray-100 dark:border-none shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden h-full flex flex-col">
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className={`text-xs px-2 py-1 rounded-full font-medium ${project.status === 'DONE' ? 'bg-status-done/10 text-status-done' :
                                    project.status === 'IN_PROGRESS' ? 'bg-status-progress/10 text-status-progress' :
                            ...
                            <div className="mt-auto pt-4 border-t border-gray-100 dark:border-border-dark flex items-center justify-between text-xs text-slate-400">
